// Страница установки нового пароля по токену из письма reset-password
"use client";
  ...
  return (
    <div className="flex min-h-full flex-col justify-center bg-gradient-to-b from-teal-50/50 via-stone-50 to-stone-100/80 px-4 py-12 dark:from-stone-950 dark:via-stone-900 dark:to-stone-950">
      <div className="mx-auto w-full max-w-sm rounded-2xl border border-stone-200/90 bg-white p-8 shadow-md shadow-stone-900/5 ring-1 ring-teal-100/50 dark:border-stone-800 dark:bg-stone-900 dark:ring-teal-900/20">
        ...
        <p className="mt-1 text-sm text-stone-500">Укажите новый пароль для вашего аккаунта.</p>
        <form
          ...
          onSubmit={async (e) => {
            ...
            setError(null);
            if (!token) {
              setError("Ссылка недействительна: не найден токен.");
              return;
            }
            setLoading(true);
            ...
            setLoading(false);
            if (!res.ok) {
              setError(typeof data.error === "string" ? data.error : "Не удалось изменить пароль");
              return;
            }
            // После успешного reset отправляем на login и показываем информационную плашку через query
